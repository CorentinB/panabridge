...
export class PanasonicBD {
    ...
    sendCmd(url: string, data: string, callback: (err: any, resp: [string, any]) => void): void {
        const options = {
            method: 'POST',
            timeout: 5000,
            headers: {
        ...
        // Debug logging: print HTTP request details
        ...
        req.on('error', (e) => {
            // If we can’t reach the device, assume it is off.
            this._state = 'off';
    ...
    sendKey(key: string, callback: (err: any, resp: [string, any]) => void): void {
    ...
    getStatus(callback: (err: any, status: any) => void): void {
        if (this._variant === "UB") {
    ...
    getPlayStatus(callback: (err: any, state: string, playtime: number, duration: number) => void): void {
        const url = `http://${this._host}/WAN/dvdr/dvdr_ctrl.cgi`;
        ...
        this.sendCmd(url, data, (err, resp) => {
            ...
            this.getStatus((err, status) => {
                ...
                const stateResponse = resp[1]; // Expected format: [stateCode, playtime, ...]
                let state: string;
